...
class ReportPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 5, "AI-Powered Coaching Insights", border=False, ln=True, align="C")
        self.ln(5)
...
def generate_report_pdf(
    ...
    result = "Win" if stats.points_for > stats.points_against else "Loss" if stats.points_for < stats.points_against else "Tie"
    pdf.cell(0, 6, f"Your Team: {stats.points_for}  |  {game.opponent_name}: {stats.points_against}  ({result})", ln=True)
    pdf.ln(5)
    ...
    # Two column stats
    col_width = 95
    pdf.cell(col_width, 6, f"Field Goals: {stats.fg_made}/{stats.fg_att} ({fg_pct:.1f}%)")
    pdf.cell(col_width, 6, f"Rebounds: {stats.rebounds_off + stats.rebounds_def} (Off: {stats.rebounds_off}, Def: {stats.rebounds_def})", ln=True)
    
    pdf.cell(col_width, 6, f"3-Pointers: {stats.three_made}/{stats.three_att} ({three_pct:.1f}%)")
    pdf.cell(col_width, 6, f"Assists: {stats.assists}", ln=True)
    
    pdf.cell(col_width, 6, f"Free Throws: {stats.ft_made}/{stats.ft_att} ({ft_pct:.1f}%)")
    ...
    pdf.cell(0, 5, f"Generated by {report.model_used or 'AI'} on {report.created_at.strftime('%B %d, %Y at %H:%M')}", ln=True, align="C")
